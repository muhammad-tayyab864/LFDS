...
class Train:
    def __init__(self,num_epochs = 80, batch_size = 1, num_workers = 0, img_shape = (450,450)):
        pc = 1
        if(pc == 0):
    ...
    def train(self,num_epochs,train_loader,eval_loader):
        ...
        optimizer = optim.Adam(race.parameters(), lr=race.learning_rate, betas=(0.9, 0.999))
        
        self.logging.info(race)
        ...
        for epoch in tqdm.tqdm(range(num_epochs)):
            ...
            for batch_idx, (dataL, dataR) in enumerate(train_loader):
                ...
                outL_y = race(dataL_y, r)
                outR_y = race(dataR_y, r)
                ...
                _TVloss = TV_loss(outL_y, dataL_y) + TV_loss(outR_y, dataR_y)
                # loss = (_ssimloss + _rloss + _contrastloss) / 2
                loss = (_ssimloss + _rloss + _contrastloss + _TVloss) / 2
                loss = loss.float()
                # Optimize
                loss.backward()
                optimizer.step()
            ...
            with torch.no_grad():
                ...
                torch.save(race.state_dict(), os.path.join(self.out_file,'last.pth'))
                if(avg_loss_val < min_loss):
                    min_loss_epoch = epoch
                    min_loss = avg_loss_val
                    torch.save(race.state_dict(), os.path.join(self.out_file,'best.pth'))
