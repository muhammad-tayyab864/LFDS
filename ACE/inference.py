...
pc = 1
dataset_id = 1 
dataset_list = ['stereoColorTransferDataset','kitti2015','middlebury2006','flickr1024']
...
class Inference:
    def __init__(self, resize=None, save=True):
        ...
        # print(len(os.listdir('C:/Users/lps3090/Desktop/Testing_StereoOLED/Datasets/Flickr1024/Test')))
        ...
        with torch.no_grad():
            for batch_idx, (inputL, inputR, filenameL, filenameR) in tqdm.tqdm(enumerate(self.test_loader)):
                ...
                while(finish != True):
                    ...
                    outL_y = torch.where(torch.isnan(outL_y), torch.tensor(0.0, dtype=outL_y.dtype, device=outL_y.device), outL_y)
                    outR_y = torch.where(torch.isnan(outR_y), torch.tensor(0.0, dtype=outR_y.dtype, device=outR_y.device), outR_y)
                    ...
                    if(abs(R - R_actual) < 0.005):
                        ...
                        count += 1
                        R_in = R_in + float(R-R_actual) * 0.7 * (1 / count + 1e-2)
