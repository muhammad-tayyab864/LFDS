...
model_path = r'C:/Users/Tayyab/Desktop/Testing_StereoOLED/proposed/exp0-orignal'
    ...
    def __init__(self, resize=None):
        ...
        if(pc == 0):
            if(dataset == dataset_list[0]):
                dataset_test_path = 'C:/Users/Tayyab/Desktop/Testing_StereoOLED/Datasets/Flickr1024/Test'
                test_dataset = testDataset(root=[dataset_test_path],transform=transform)
            
            elif(dataset == dataset_list[1]):
                dataset_test_path = r'C:/Users/Tayyab/Desktop/Testing_StereoOLED/Datasets/KITTI/test'
                test_dataset = testDataset(root=[dataset_test_path],transform=transform)
                ...
                dataset_test_path = r'C:/Users/Tayyab/Desktop/Testing_StereoOLED/Datasets/middleburry2014/MiddEval3/test/unified'
                test_dataset = middleburyTestDataset(root=[dataset_test_path],transform=transform)

        elif(pc == 1):
            if(dataset == dataset_list[0]):
                dataset_test_path = 'C:/Users/Tayyab/Desktop/Testing_StereoOLED/Datasets/stereoColorTransferDataset/Test'
                test_dataset = testDataset(root=[dataset_test_path],transform=transform)
        ...
        save_path = r'C:/Users/Tayyab/Desktop/Testing_StereoOLED/proposed/results'
        ...
        # ==========================================================
# Sensitivity Study Containers
# ==========================================================

        self.gamma_results = {}

        for g in gamma_values:
            self.gamma_results[g] = []
        # ==========================================================
# Coefficient Sensitivity Containers
# ==========================================================
    ...
    def inference(self):        
        ...
        with torch.no_grad():
            ...
            # ==========================================================
                    ...
                    p_in = (
                        oled_power(inputL_rgb_np, nominal_ar, nominal_ag, nominal_ab, 2.2)
                        + oled_power(inputR_rgb_np, nominal_ar, nominal_ag, nominal_ab, 2.2)
                    )

                    p_out = (
                        oled_power(outL_rgb_np, nominal_ar, nominal_ag, nominal_ab, 2.2)
                        + oled_power(outR_rgb_np, nominal_ar, nominal_ag, nominal_ab, 2.2)
                    )
                    ...
                    for gamma in gamma_values:
                        p_in_gamma = (
                            oled_power(inputL_rgb_np, nominal_ar, nominal_ag, nominal_ab, gamma)
                            + oled_power(inputR_rgb_np, nominal_ar, nominal_ag, nominal_ab, gamma)
                        )

                        p_out_gamma = (
                            oled_power(outL_rgb_np, nominal_ar, nominal_ag, nominal_ab, gamma)
                            + oled_power(outR_rgb_np, nominal_ar, nominal_ag, nominal_ab, gamma)
                        )
...
# ==========================================================
                    ...
                    for p in perturb_values:
                        ...
                        p_in_red = (
                            oled_power(inputL_rgb_np, ar, nominal_ag, nominal_ab, 2.2)
                            + oled_power(inputR_rgb_np, ar, nominal_ag, nominal_ab, 2.2)
    )

                        p_out_red = (
                            oled_power(outL_rgb_np, ar, nominal_ag, nominal_ab, 2.2)
                            + oled_power(outR_rgb_np, ar, nominal_ag, nominal_ab, 2.2)
    )

                        saving_red = 100 * (1 - p_out_red / p_in_red)

                        self.red_results[p].append(saving_red)
...
# ==========================================================
                    ...
                    for p in perturb_values:
                        ...
                        p_in_green = (
                            oled_power(inputL_rgb_np, nominal_ar, ag, nominal_ab, 2.2)
                            + oled_power(inputR_rgb_np, nominal_ar, ag, nominal_ab, 2.2)
    )

                        p_out_green = (
                            oled_power(outL_rgb_np, nominal_ar, ag, nominal_ab, 2.2)
                            + oled_power(outR_rgb_np, nominal_ar, ag, nominal_ab, 2.2)
    )

                        saving_green = 100 * (1 - p_out_green / p_in_green)

                        self.green_results[p].append(saving_green)
...
# ==========================================================
                    ...
                    for p in perturb_values:
                        ...
                        p_in_blue = (
                            oled_power(inputL_rgb_np, nominal_ar, nominal_ag, ab, 2.2)
                            + oled_power(inputR_rgb_np, nominal_ar, nominal_ag, ab, 2.2)
    )

                        p_out_blue = (
                            oled_power(outL_rgb_np, nominal_ar, nominal_ag, ab, 2.2)
                            + oled_power(outR_rgb_np, nominal_ar, nominal_ag, ab, 2.2)
    )

                        saving_blue = 100 * (1 - p_out_blue / p_in_blue)

                        self.blue_results[p].append(saving_blue)
...
for gamma in gamma_values:
    ...
    vals = np.array(runner.gamma_results[gamma])
...
for p in perturb_values:
    ...
    vals = np.array(runner.red_results[p])
...
for p in perturb_values:
    ...
    vals = np.array(runner.green_results[p])
...
for p in perturb_values:
    ...
    vals = np.array(runner.blue_results[p])
