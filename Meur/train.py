...
from Model import Meur
from Loss import ssim_loss, r_loss, MAE_loss, TV_loss
...
class Train:
    def __init__(self,num_epochs = 80, batch_size = 4, num_workers = 0, img_shape = (450,450)):
        pc = 1
        if(pc == 0):
    ...
    def train(self,num_epochs,train_loader,eval_loader):
        csa = Meur().to(self.device)
        optimizer = optim.Adam(csa.parameters(), lr=1e-3, weight_decay=1e-5)
        
        self.logging.info(csa)
        ...
        for epoch in tqdm.tqdm(range(num_epochs)):
            ...
            for batch_idx, (dataL, dataR) in enumerate(train_loader):
                ...
                outL_y = csa(dataL_y, r)
                outR_y = csa(dataR_y, r)

                outL = torch.clamp(compose_imgs(outL_y, cbcrL), 0, 1)
                outR = torch.clamp(compose_imgs(outR_y, cbcrR), 0, 1)

                # Train Loss  
                _ssimloss = ssim_loss(outL_y, dataL_y) + ssim_loss(outR_y, dataR_y)
                _rloss    =  r_loss(outL_y, dataL_y, r) + r_loss(outR_y, dataR_y, r)
                _TVloss   = TV_loss(outL_y) + TV_loss(outR_y)
                _MAEloss  = MAE_loss(outL_y, dataL_y) + MAE_loss(outR_y, dataR_y)

                if(epoch < 2):
                    loss  = (_ssimloss + _MAEloss) / 2
                else:
                    loss  = (_ssimloss + _rloss + _TVloss + _MAEloss) / 2
                # Optimize
                loss.backward()
                optimizer.step()
                ...
                MAE_loss_train += _MAEloss.item()
                count_train += 1
                avg_loss_train = (ssim_loss_train + r_loss_train + TV_loss_train + MAE_loss_train)/count_train
            ...
            with torch.no_grad():
                ...
                for batch_idx, (dataL, dataR) in enumerate(eval_loader):
                    ...
                    MAE_loss_val += _MAEloss.item()
                    count_val += 1
                    avg_loss_val = (ssim_loss_val + r_loss_val + TV_loss_val + MAE_loss_val)/count_val

                ### save and log
                train_list.append([ssim_loss_train/count_train, r_loss_train/count_train, TV_loss_train/count_train, MAE_loss_train/count_train])
                val_list.append([ssim_loss_val/count_val, r_loss_val/count_val, TV_loss_val/count_val, MAE_loss_val/count_val])

                torch.save(csa.state_dict(), os.path.join(self.out_file,'last.pth'))
                if(avg_loss_val < min_loss):
                    min_loss_epoch = epoch
                    min_loss = avg_loss_val
                    torch.save(csa.state_dict(), os.path.join(self.out_file,'best.pth'))
                ...
                print(f'MAE   Eval Loss:{MAE_loss_val/count_val:.4f}')
                ...
                self.logging.info(f'MAE   Eval Loss:{MAE_loss_val/count_val:.4f}')
        ...
        TV_train = [train_list[i][2] for i in range(len(train_list))]
        TV_val = [val_list[i][2] for i in range(len(val_list))]
        MAE_train = [train_list[i][3] for i in range(len(train_list))]
        MAE_val = [val_list[i][3] for i in range(len(val_list))]
        ## SSIM Loss
        plt.subplot(221)
        ...
        ## MAE Loss
        plt.subplot(224)
        plt.plot(t,MAE_train,label='MAE train')
        plt.plot(t,MAE_val,label='MAE val')
        plt.title('MAE Loss')
        plt.legend()
